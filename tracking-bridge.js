(function installGaietyTrackingBridge() {
  ...
  function creativeContext() {
    return compactObject({
      ...
      sales_message: params.get("sales_message") || params.get("ct_sales_message"),
      format: params.get("format") || params.get("ct_format"),
      angle: params.get("angle") || params.get("ct_angle"),
      awareness_level: params.get("awareness_level") || params.get("ct_awareness_level"),
  ...
  function readPageInstanceId(tracker) {
    if (!tracker) return null;
    return typeof tracker.getPageInstanceId === "function" ? tracker.getPageInstanceId() : null;
  }
  ...
  function stateProperties(state) {
    const now = Date.now();
    const activeVisibleMs = state.visibleStartedAt ? now - state.visibleStartedAt : 0;
    return Object.assign({}, PRODUCT, creativeContext(), rectContext(state.link), {
      placement: state.placement,
      cta_id: state.ctaId,
      element_text: (state.link.textContent || "").trim().replace(/\s+/g, " ").slice(0, 200) || null,
      element_class: state.link.className || null,
      seconds_elapsed: Math.max(0, Math.round((now - PAGE_STARTED_AT) / 1000)),
      ...
      integration_version: "gaiety-bridge-v2",
  ...
  function installCreativeContext(tracker) {
    ...
    tracker.track("creative_context", Object.assign({}, PRODUCT, creativeContext(), {
      integration_version: "gaiety-bridge-v2",
      tracker_channel: usePreviewTracker ? "preview" : "production",
    }));
  }
  ...
  function installCtaExposure(tracker) {
    ...
    function stopVisibility(state, reason) {
      ...
      if (reason === "pagehide" && state.totalVisibleMs > 0) {
        tracker.track("cta_visibility_summary", Object.assign(stateProperties(state), {
          reason,
    ...
    document.addEventListener("visibilitychange", () => {
      if (document.visibilityState === "hidden") {
        ctaStates.forEach((state) => stopVisibility(state, "visibility_hidden"));
      }
    });
