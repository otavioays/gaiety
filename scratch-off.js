(function installGaietyScratchOffer(){
  ...
  const OFFER_STYLE_MARKER="data-gaiety-scratch-offer-style";
  const MODAL_SELECTOR="[data-scratch-modal]";
  const META_PIXEL_ID="1725992278652713";
  const PRODUCT_ID="15917657129329";
  const STORE_HOST="tedyzw-27.myshopify.com";
  const ORIGINAL_VARIANT_ID="64223935299953";
  const REWARD_VARIANT_ID="64223935234417";
  const ORIGINAL_UNITS=2;
  const REWARD_UNITS=5;
  const ORIGINAL_PRICE=156;
  const REWARD_PRICE=252;
  const CAMPAIGN_VERSION="scratch-reset-v2";
  const REWARD_CHECKOUT_URL=`https://${STORE_HOST}/cart/${REWARD_VARIANT_ID}:1?checkout`;
  ...
  let state={
    ...
    trigger:null,
    originalCheckoutUrl:"",
    rewardCheckoutUrl:REWARD_CHECKOUT_URL,
    checkoutId:"",
  ...
  function loadStyle(){
    appendStyle(`scratch-off.css?v=3-${Date.now()}`,STYLE_MARKER);
    appendStyle(`scratch-off-offer.css?v=2-${Date.now()}`,OFFER_STYLE_MARKER);
  }
  ...
  function trackScratchEvent(name,extra={}){
    const properties={
      ...
      reward_variant_id:REWARD_VARIANT_ID,
      original_offer_units:ORIGINAL_UNITS,
      reward_offer_units:REWARD_UNITS,
      original_offer_price:ORIGINAL_PRICE,
      reward_offer_price:REWARD_PRICE,
      currency:"BRL",
      placement:"final_offer",
      scratch_reward:"buy_3_get_2_free",
      scratch_campaign:CAMPAIGN_VERSION,
  ...
  function trackInitiateCheckout(properties){
    const eventId=createEventId("checkout");
    const customData={
      currency:"BRL",
      value:properties.value,
      content_ids:[properties.variant_id],
      content_type:"product",
      content_name:properties.offer_name,
      content_category:"Foco e bem-estar",
      num_items:properties.quantity
    };
  ...
  function modalMarkup(){
    return `<div class="scratch-modal" data-scratch-modal hidden>
      <section class="scratch-dialog" role="dialog" aria-modal="true" aria-labelledby="scratch-title" aria-describedby="scratch-description">
        ...
        <div class="scratch-dialog__content">
          <div class="scratch-dialog__copy">
            <div class="scratch-brand" aria-label="GAIETY Modo Claro"><strong>GAIETY</strong><span>MODO CLARO</span></div>
            <span class="scratch-kicker">BENEFÍCIO DESBLOQUEADO</span>
            <h2 id="scratch-title">Raspe para revelar seu presente</h2>
            <p id="scratch-description">Sua escolha desbloqueou uma condição exclusiva antes do checkout.</p>
            <div class="scratch-ticket-wrap">
              <div class="scratch-ticket" data-scratch-ticket>
                <div class="scratch-reward" aria-live="polite">
                  <span>VOCÊ DESBLOQUEOU</span>
                  <strong>2 GRÁTIS</strong>
                  <small>Compre 3 pacotes e receba 5 no total</small>
                </div>
            ...
            <button class="scratch-claim" type="button" data-scratch-claim disabled>Quero pagar 3 e levar 5 <span>→</span></button>
            <button class="scratch-skip" type="button" data-scratch-skip>Continuar com o kit de 2</button>
          </div>
          <div class="scratch-dialog__visual scratch-dialog__visual--offer" aria-hidden="true">
            <div class="scratch-product-halo scratch-product-halo--offer">
              <img src="${REWARD_IMAGE_URL}" alt="" width="1102" height="1378" decoding="async">
            </div>
          </div>
        </div>
      </section>
  ...
  function checkProgress(canvas,force){
    ...
    if(sampled&&cleared/sampled>=REVEAL_THRESHOLD) finishReveal("scratch");
  }
  ...
  function finishReveal(method){
    ...
    canvas.setAttribute("aria-label","Benefício revelado: pague por 3 pacotes e receba 5 no total.");
    state.modal.classList.add("is-revealed");
    claim.disabled=false;
    if(instruction) instruction.textContent="Benefício revelado. Sua oferta de 5 pacotes pagando 3 está pronta.";

    trackScratchEvent("scratch_reveal",{reveal_method:method});
    window.setTimeout(()=>claim.focus(),260);
  }

  function buildProperties({variantId,units,price,offerName,offerType}){
    return {
      product_id:PRODUCT_ID,
  ...
  function openModal(trigger){
    ...
    state={
      modal,
      trigger,
      originalCheckoutUrl:trigger.dataset.checkoutUrl||"",
      rewardCheckoutUrl:REWARD_CHECKOUT_URL,
      checkoutId,
      originalProperties:null,
      rewardProperties:null,
    ...
    state.originalProperties=buildProperties({
      variantId:ORIGINAL_VARIANT_ID,
      units:ORIGINAL_UNITS,
      price:ORIGINAL_PRICE,
      offerName:"Mushroom Complex - 2 unidades",
      offerType:"original_2_units"
    });
    state.rewardProperties=buildProperties({
      variantId:REWARD_VARIANT_ID,
      units:REWARD_UNITS,
      price:REWARD_PRICE,
      offerName:"Mushroom Complex - pague 3 e leve 5",
      offerType:"buy_3_get_2_free"
    });
    ...
    void trackerEvent("buy_button_click",state.originalProperties,{beacon:true});
  ...
  function proceedToCheckout(action){
    ...
    const eventName=isClaim?"scratch_redeem":"scratch_skip";
    const properties=isClaim?state.rewardProperties:state.originalProperties;
    const targetUrl=isClaim?state.rewardCheckoutUrl:state.originalCheckoutUrl;

    trackScratchEvent(eventName,{
      scratch_completed:state.revealed,
      selected_offer:properties.offer_type,
      selected_variant_id:properties.variant_id,
      selected_units:properties.offer_units,
      selected_value:properties.value
    });
    trackInitiateCheckout(properties);

    const button=state.modal.querySelector(isClaim?"[data-scratch-claim]":"[data-scratch-skip]");
    if(button){
      button.disabled=true;
      button.classList.add("is-loading");
    }

    const checkoutUrl=buildTrackedCheckoutUrl(targetUrl,properties);
    if(checkoutUrl){
